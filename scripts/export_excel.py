...
def build_workbook(data: dict) -> Workbook:
    ...
    summary.append([])
    summary.append(["Realizadas", None, "Aplican", None, "% Avance", None, None, None])
    ...
    headers = ["Ranking", "DM", "Tiendas", "Realizadas", "Aplican", "No aplica", "% Avance", "Estado"]
    ...
    for rank, item in enumerate(dms, 1):
        row = 9 + rank
        values = [rank, item.get("shortName"), item.get("stores", 0), item.get("completed", 0), item.get("expected", 0), item.get("notApplicable", 0), f"=IFERROR(D{row}/E{row},0)", status(item.get("compliance", 0))]
    ...
    set_widths(summary, [10, 28, 12, 14, 12, 12, 14, 16])
    ...
    stores_headers = ["Ranking", "CeCo", "Tienda", "DM", "Realizadas", "Aplican", "No aplica", "% Avance", "Estado"]
    ...
    for rank, item in enumerate(stores, 1):
        ...
        stores_sheet.append([rank, item.get("ceco"), item.get("store"), item.get("dm"), item.get("completed", 0), item.get("expected", 0), item.get("notApplicable", 0), f"=IFERROR(E{row}/F{row},0)", status(item.get("compliance", 0))])
    ...
    set_widths(stores_sheet, [10, 12, 30, 32, 14, 12, 12, 14, 16])
    ...
    activity_headers = ["Orden", "Actividad", "Realizadas", "Aplican", "No aplica", "% Avance", "Fecha compromiso"]
    ...
    for item in activities:
        ...
        activities_sheet.append([item.get("order"), item.get("name"), item.get("completedStores", 0), item.get("applicableStores", 0), item.get("notApplicableStores", 0), f"=IFERROR(C{row}/D{row},0)", item.get("commitmentDateDisplay", "Sin fecha")])
    ...
    set_widths(activities_sheet, [10, 42, 14, 12, 12, 14, 20])
