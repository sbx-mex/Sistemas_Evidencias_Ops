...
def build_workbook(data: dict) -> Workbook:
    ...
    summary.append([])
    summary.append(["Realizadas", None, "Total", None, "% Avance", None, None])
    summary.append([source.get("completedCompletions", 0), None, source.get("expectedCompletions", 0), None, None, None, None])
    ...
    summary["E6"] = "=IFERROR(A6/C6,0)"
    summary["E6"].number_format = "0.0%"

    headers = ["Ranking", "DM", "Tiendas", "Realizadas", "Total", "% Avance", "Estado"]
    ...
    for rank, item in enumerate(dms, 1):
        row = 9 + rank
        values = [rank, item.get("shortName"), item.get("stores", 0), item.get("completed", 0), item.get("expected", 0), f"=IFERROR(D{row}/E{row},0)", status(item.get("compliance", 0))]
        for column, value in enumerate(values, 1):
            summary.cell(row=row, column=column, value=value)
    style_header(summary, 9, 1, 7)
    style_table(summary, 10, summary.max_row, 7)
    summary.freeze_panes = "A10"
    summary.auto_filter.ref = f"A9:G{summary.max_row}"
    ...
    set_widths(summary, [10, 28, 12, 14, 12, 14, 16])

    summary.print_area = f"A1:G{summary.max_row}"
    summary.page_setup.orientation = "landscape"
    summary.page_setup.fitToWidth = 1

    style_title(stores_sheet, 8, "Avance por tienda", f"{region} · Corte {cut}")
    stores_headers = ["Ranking", "CeCo", "Tienda", "DM", "Realizadas", "Total", "% Avance", "Estado"]
    ...
    for rank, item in enumerate(stores, 1):
        ...
        stores_sheet.append([rank, item.get("ceco"), item.get("store"), item.get("dm"), item.get("completed", 0), item.get("expected", 0), f"=IFERROR(E{row}/F{row},0)", status(item.get("compliance", 0))])
    style_header(stores_sheet, 4, 1, 8)
    style_table(stores_sheet, 5, stores_sheet.max_row, 8)
    stores_sheet.freeze_panes = "A5"
    stores_sheet.auto_filter.ref = f"A4:H{stores_sheet.max_row}"
    ...
    set_widths(stores_sheet, [10, 12, 30, 32, 14, 12, 14, 16])
    stores_sheet.print_title_rows = "1:4"
    stores_sheet.page_setup.orientation = "landscape"
    stores_sheet.page_setup.fitToWidth = 1

    style_title(activities_sheet, 6, "Avance por actividad", f"{region} · Corte {cut}")
    activity_headers = ["Orden", "Actividad", "Realizadas", "Total", "% Avance", "Fecha compromiso"]
    ...
    for item in activities:
        ...
        activities_sheet.append([item.get("order"), item.get("name"), item.get("completedStores", 0), item.get("completedStores", 0) + item.get("pendingStores", 0), f"=IFERROR(C{row}/D{row},0)", item.get("commitmentDateDisplay", "Sin fecha")])
    style_header(activities_sheet, 4, 1, 6)
    style_table(activities_sheet, 5, activities_sheet.max_row, 6)
    activities_sheet.freeze_panes = "A5"
    activities_sheet.auto_filter.ref = f"A4:F{activities_sheet.max_row}"
    ...
    set_widths(activities_sheet, [10, 42, 14, 12, 14, 20])
