...
def style_title(ws, end_column: int, title: str, subtitle: str) -> None:
    ...
    title_fill = PatternFill("solid", fgColor=GREEN)
    for row in ws.iter_rows(min_row=1, max_row=2, min_col=1, max_col=end_column):
        for cell in row:
            cell.fill = title_fill
            cell.font = Font(name="Aptos Display", size=21, bold=True, color=WHITE)
            cell.alignment = Alignment(horizontal="center", vertical="center")
    ws.merge_cells(f"A1:{end}2")
    ws["A1"] = title
    for cell in ws[3][:end_column]:
...
def style_decision_column(ws, start_row: int, end_row: int, column: int) -> None:
    palette = {
        "Mantener estándar": ("DAF1E6", "116444"),
        "Dar seguimiento": ("FFF0D5", "80520C"),
        "Priorizar hoy": ("F9E6E3", "922F24"),
    }
    for row in range(start_row, end_row + 1):
        cell = ws.cell(row=row, column=column)
        fill, font = palette.get(cell.value, (CANVAS, DARK))
...
def build_workbook(data: dict) -> Workbook:
    ...
    summary["E6"] = "=IFERROR(A6/(A6+C6),0)"
    summary["E6"].number_format = "0.0%"

    headers = ["Ranking", "DM", "Tiendas", "Realizadas", "Pendientes", "% Avance", "Estado", "Decisión"]
    ...
    for rank, item in enumerate(dms, 1):
        row = 9 + rank
        values = [rank, item.get("shortName"), item.get("stores", 0), item.get("completed", 0), item.get("pending", 0), f"=IFERROR(D{row}/(D{row}+E{row}),0)", status(item.get("compliance", 0)), decision(item.get("compliance", 0))]
    ...
    summary.conditional_formatting.add(f"F10:F{summary.max_row}", DataBarRule(start_type="num", start_value=0, end_type="num", end_value=1, color=GREEN, showValue=True))
    for row in range(10, summary.max_row + 1):
        for column in (1, 3, 4):
            summary.cell(row=row, column=column).number_format = "#,##0"
        summary[f"F{row}"].number_format = "0.0%"
    style_status_column(summary, 10, summary.max_row, 7)
    style_decision_column(summary, 10, summary.max_row, 8)
    set_widths(summary, [10, 28, 12, 14, 14, 14, 16, 20])
    ...
    stores_headers = ["Ranking", "CeCo", "Tienda", "DM", "Realizadas", "Pendientes", "% Avance", "Estado", "Decisión"]
    ...
    for rank, item in enumerate(stores, 1):
        ...
        stores_sheet.append([rank, item.get("ceco"), item.get("store"), item.get("dm"), item.get("completed", 0), item.get("expected", 0) - item.get("completed", 0), f"=IFERROR(E{row}/(E{row}+F{row}),0)", status(item.get("compliance", 0)), decision(item.get("compliance", 0))])
    ...
    stores_sheet.conditional_formatting.add(f"G5:G{stores_sheet.max_row}", DataBarRule(start_type="num", start_value=0, end_type="num", end_value=1, color=GREEN, showValue=True))
    for row in range(5, stores_sheet.max_row + 1):
        ...
        stores_sheet[f"G{row}"].number_format = "0.0%"
    style_status_column(stores_sheet, 5, stores_sheet.max_row, 8)
    style_decision_column(stores_sheet, 5, stores_sheet.max_row, 9)
    set_widths(stores_sheet, [10, 12, 28, 30, 14, 14, 14, 16, 20])
    stores_sheet.print_title_rows = "1:4"
    stores_sheet.page_setup.orientation = "landscape"
    stores_sheet.page_setup.fitToWidth = 1

    style_title(activities_sheet, 8, "Avance por actividad", f"{region} · Corte {cut}")
    activity_headers = ["Orden", "Actividad", "Realizadas", "Pendientes", "% Avance", "Fecha compromiso", "Estado", "Decisión"]
    ...
    for item in activities:
        ...
        activities_sheet.append([item.get("order"), item.get("name"), item.get("completedStores", 0), item.get("pendingStores", 0), f"=IFERROR(C{row}/(C{row}+D{row}),0)", item.get("commitmentDateDisplay", "Sin fecha"), status(compliance), decision(compliance)])
    style_header(activities_sheet, 4, 1, 8)
    style_table(activities_sheet, 5, activities_sheet.max_row, 8)
    activities_sheet.freeze_panes = "A5"
    activities_sheet.auto_filter.ref = f"A4:H{activities_sheet.max_row}"
    activities_sheet.conditional_formatting.add(f"E5:E{activities_sheet.max_row}", DataBarRule(start_type="num", start_value=0, end_type="num", end_value=1, color=GREEN, showValue=True))
    for row in range(5, activities_sheet.max_row + 1):
        ...
        activities_sheet[f"E{row}"].number_format = "0.0%"
    style_status_column(activities_sheet, 5, activities_sheet.max_row, 7)
